...
export const CustomerWorkspaceSidebar: React.FC<CustomerWorkspaceSidebarProps> = ({
  ...
  return (
    <div className={`w-[230px] bg-white border-r border-[#E5E2DC] py-6 px-4 flex flex-col justify-between h-full ${className}`}>
      <div className="space-y-5">
        {/* Simple Brand Header */}
        <div className="px-2 pb-2 border-b border-[#EAE7E1]">
          <span className="text-[11px] font-mono font-bold uppercase tracking-wider text-[#B5652D] block">
            EZFINANZ CUSTOMER
          </span>
        ...
        <nav className="space-y-1">
          {navItems.map((item) => {
            const isCurrent =
              ...
              (item.id === 'applications' && (location.pathname === '/applications' || location.hash === '#applications')) ||
